...
reg  [31:0] mem_wr_line [LINE_SIZE]; // 要写入的line数据，这是一个二维数组
wire [31:0] mem_rd_line [LINE_SIZE]; // 要读出的line数据，这是一个二维数组
...
always @ (posedge clk or posedge rst) begin     //
    if(rst) begin
        ...
        IDLE:       begin
                        if(cache_hit) begin
                            ...
                            if(wr_req | rd_req) begin   // 如果 cache 未命中，并且有读写请求，则需要进行换入
                                if(valid[set_addr][fifo_index[set_addr]] & dirty[set_addr][fifo_index[set_addr]]) begin    // 如果 要换入的cache line 本来有效，且脏，则需要先将它换出
                                    cache_stat  <= SWAP_OUT;
                                    mem_wr_addr <= { cache_tags[set_addr][fifo_index[set_addr]], set_addr };
                                    mem_wr_line <= cache_mem[set_addr][fifo_index[set_addr]];
        ...
        SWAP_IN_OK: begin           // 上一个周期换入成功，这周期将主存读出的line写入cache，并更新tag，置高valid，置低dirty
                        for(integer i=0; i<LINE_SIZE; i++)  cache_mem[mem_rd_set_addr][fifo_index[set_addr]][i] <= mem_rd_line[i];
                        cache_tags[mem_rd_set_addr][fifo_index[set_addr]] <= mem_rd_tag_addr;
                        valid     [mem_rd_set_addr][fifo_index[set_addr]] <= 1'b1;
                        dirty     [mem_rd_set_addr][fifo_index[set_addr]] <= 1'b0;
                        fifo_index[set_addr]<=(fifo_index[set_addr]+1)%WAY_CNT;
    ...
    if(rst) begin
        ...
        for(integer i=0; i<SET_SIZE; i++) begin
            for (integer j=0; j<WAY_CNT; j++) begin
                ...
                valid[i][j] = 1'b0;
                lru_queue[i][j] = j;
                // lru_queue[i][j*BIT_FOR_WAY_CNT:(j+1)*BIT_FOR_WAY_CNT-1]=j;
            end
        end
        for(integer k = 0; k < LINE_SIZE; k++)
            mem_wr_line[k] <= 0;
...
reg [1:0] lru_queue[SET_SIZE][WAY_CNT];
// 下面是判定是否命中逻辑
