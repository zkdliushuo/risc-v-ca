...
module ControllerDecoder(
    ...
    // 当是system指令(不考虑ecall和ebreak) 或者是csrrw或者csrrwi指令 并且rd不是0,就去读csr
    assign CSR_read_en=(inst[6:0]==7'b1110011 || ({inst[13:12],inst[6:0]}==9'b011110011 && inst[11:7]!=5'b00000))?1'b1:1'b0;
    // 当是system指令(不考虑ecall和ebreak) 或者是csrrs(i)或者csrrc(i)指令 并且rs1(zimm)不是0,就去写csr
    assign CSR_write_en=(inst[6:0]==7'b1110011 || ({inst[13],inst[6:0]}==8'b11110011 && inst[19:15]!=5'b00000))?1'b1:1'b0;    
    // 当是system指令(不考虑ecall和ebreak) ALUout选择CSR读出值(为了借用转发和写回寄存器的路线)
    assign ALU_out_sel=(inst[6:0]==7'b1110011)?1'b1:1'b0;
    // CSR_func[1:0]用来区分CSR cal的操作 CSR_func[2]用来识别选择rs1还是zimm
